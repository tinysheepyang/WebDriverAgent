...
export class PhotoProxyServer {
    constructor(config = {}) {
        ...
        this.thumbnailManager = new ThumbnailManager(this.photosService, this.memoryCache, this.diskCache);
        // 定期清理过期缓存
        setInterval(() => {
     ...
     */
    async handleGetAssets(req, res, query) {
        try {
     ...
     */
    async handleGetThumbnail(req, res, assetId, query) {
        const startTime = Date.now();
        try {
            const size = query.size ? parseInt(query.size, 10) : 320;
     ...
     */
    async handleGetAssetImage(req, res, assetId, query) {
        const startTime = Date.now();
        try {
            const quality = query.quality || 'full'; // full | screen
            const format = query.format || 'jpeg'; // jpeg | heic
            ...
            const imageStream = await this.photosService.getImage(assetId, quality);
            // 设置 Content-Type
     ...
     */
    async handleGetAssetVideoStream(req, res, assetId) {
        const startTime = Date.now();
        try {
