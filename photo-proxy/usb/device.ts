...
export interface IOSDevice {
  udid: string
  name?: string
  type?: string // 设备类型，如 'iPhone', 'iPad'
  productType?: string
  iosVersion?: string
  version?: string // 兼容字段
}
...
export class DeviceManager {
  private iosCommandPath: string
  private useLibimobiledevice: boolean = false
   ...
   */
  private checkLibimobiledevice(): void {
    // 异步检查，不阻塞构造函数
      ...
      if (this.useLibimobiledevice) {
        ...
        proc.on('close', (code) => {
          if (code === 0) {
            ...
            const devices: IOSDevice[] = udids.map(udid => ({
              udid: udid.trim(),
              name: undefined,
   ...
   * 使用 go-ios 获取设备列表
   */
  private getDevicesWithGoIOS(resolve: (devices: IOSDevice[]) => void, reject: (error: Error) => void): void {
    // 检查命令文件是否存在
    try {
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        const devices: IOSDevice[] = deviceList.map((udid: string) => ({
          udid: String(udid),
   ...
   * 获取第一个可用设备
   */
  async getFirstDevice(): Promise<IOSDevice | null> {
