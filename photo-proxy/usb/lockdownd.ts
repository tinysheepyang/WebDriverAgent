...
export interface LockdowndService {
  name: string
  port: number
  socket: Socket
}
...
export class LockdowndClient {
  ...
  constructor(usbmuxd: UsbmuxdClient, udid: string) {
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      if ((this.socket as any).readyState === 'open') {
    ...
    console.log('[LockdowndClient] Socket connected, readyState:', (this.socket as any).readyState)
   ...
   * 发送 plist 消息
   */
  private async sendPlistMessage(message: any, timeout: number = 10000): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // 检查 socket 状态
      if (!this.socket || (this.socket as any).readyState !== 'open') {
        clearTimeout(timer)
        this.pendingRequests.delete(requestId)
        reject(new Error(`Socket not ready, state: ${(this.socket as any)?.readyState || 'null'}`))
        return
      }
    ...
    try {
      ...
      if (helloResponse.EnableSessionSSL || helloResponse.Error === 'PairingDialogResponsePending') {
        ...
        if (pairRecord) {
          ...
          const pairMessage: any = {
            MessageType: 'PairRecord'
          }
          
          // 尝试解析配对记录 plist
          try {
            const pairPlist = plist.parse(pairRecord) as any
   ...
   * 读取配对记录
   */
  private async readPairRecord(): Promise<string | null> {
    try {
      // 配对记录通常存储在：
   ...
   */
  async startService(serviceName: string): Promise<LockdowndService> {
    if (!this.connected) {
      throw new Error('Not connected to lockdownd')
    }
