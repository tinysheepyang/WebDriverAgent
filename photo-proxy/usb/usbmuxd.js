...
// usbmuxd 消息类型
var UsbmuxdMessageType;
(function (UsbmuxdMessageType) {
    UsbmuxdMessageType[UsbmuxdMessageType["Result"] = 1] = "Result";
    UsbmuxdMessageType[UsbmuxdMessageType["Connect"] = 2] = "Connect";
    UsbmuxdMessageType[UsbmuxdMessageType["Listen"] = 3] = "Listen";
    UsbmuxdMessageType[UsbmuxdMessageType["Add"] = 4] = "Add";
    UsbmuxdMessageType[UsbmuxdMessageType["Remove"] = 5] = "Remove";
    UsbmuxdMessageType[UsbmuxdMessageType["Packet"] = 6] = "Packet";
    UsbmuxdMessageType[UsbmuxdMessageType["PlistPacket"] = 8] = "PlistPacket";
    UsbmuxdMessageType[UsbmuxdMessageType["ListDevices"] = 9] = "ListDevices";
    UsbmuxdMessageType[UsbmuxdMessageType["ListListeners"] = 10] = "ListListeners";
    UsbmuxdMessageType[UsbmuxdMessageType["ReadBUID"] = 11] = "ReadBUID";
    UsbmuxdMessageType[UsbmuxdMessageType["ReadPairRecord"] = 12] = "ReadPairRecord";
    UsbmuxdMessageType[UsbmuxdMessageType["SavePairRecord"] = 13] = "SavePairRecord";
    UsbmuxdMessageType[UsbmuxdMessageType["DeletePairRecord"] = 14] = "DeletePairRecord";
    UsbmuxdMessageType[UsbmuxdMessageType["Plist"] = 15] = "Plist";
})(UsbmuxdMessageType || (UsbmuxdMessageType = {}));
export class UsbmuxdClient {
            ...
            if (pending) {
                ...
                try {
                    ...
                    if (plistStr.includes('<key>MessageType</key>') && plistStr.includes('<key>Number</key>')) {
                        const plistData = plist.parse(plistStr);
     ...
     */
    serializeMessage(message, payload, tag) {
     ...
     * 发送消息并等待响应
     */
    async sendMessage(message, payload, timeout = 5000) {
        ...
        try {
            ...
            if (error instanceof Error && error.message.includes('timeout')) {
                ...
                try {
                    ...
                    const plistData = plist.parse(response.payload.toString('utf-8'));
                    if (plistData && plistData.DeviceList) {
                        const devices = Array.isArray(plistData.DeviceList)
                            ? plistData.DeviceList
                            : [plistData.DeviceList];
     ...
     * 连接到设备的指定端口
     */
    async connectToDevice(udid, port) {
        ...
        try {
            ...
            const connectionType = (port === 62078 || port === 0) ? 'lockdownd' : 'custom';
     ...
     */
    sendPacketMessage(connectionId, data) {
        if (!this.socket || !this.connected) {
        ...
        return new Promise((resolve) => {
            ...
            proc.on('close', (code) => {
                if (code === 0 && stdout.trim()) {
                    ...
                    const devices = udids.map((udid, index) => ({
                        DeviceID: index + 1,
                ...
                if (code === 0) {
                    try {
                        ...
                        if (Array.isArray(deviceList) && deviceList.length > 0) {
                            ...
                            const devices = deviceList.map((udid, index) => ({
                                DeviceID: index + 1,
                                MessageType: 'Attached',
                                Properties: {
