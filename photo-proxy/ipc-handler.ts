...
export function initPhotoProxyIPC(): void {
  ...
  // 获取资源列表
  ipcMain.handle('photo-proxy:list-assets', async (event, limit: number, offset: number) => {
    try {
  ...
  // 获取缩略图（返回 base64）
  ipcMain.handle('photo-proxy:get-thumbnail', async (event, assetId: string, size: number) => {
    try {
  ...
  // 获取图片（流式，返回 base64）
  ipcMain.handle('photo-proxy:get-image', async (event, assetId: string, quality: 'full' | 'screen' = 'full') => {
    try {
        ...
        stream.on('end', () => {
          ...
          if (firstBytes.length >= 12) {
            ...
            if (ftypIndex >= 0 && ftypIndex < 8) {
              ...
              const hasHeic = heicIdentifiers.some(id => 
                firstBytes.indexOf(Buffer.from(id), ftypIndex) >= 0
              )
              if (hasHeic) {
                mimeType = 'image/heic'
  ...
  // 获取视频流（流式，返回 base64）
  ipcMain.handle('photo-proxy:get-video-stream', async (event, assetId: string) => {
    try {
  ...
  // 设置设备（URL Service）
  ipcMain.handle('url-service:set-device', async (event, device: IOSDevice) => {
    try {
  ...
  // 打开 URL
  ipcMain.handle('url-service:open-url', async (event, url: string) => {
    try {
  ...
  // 设备侧 HTTP 请求
  ipcMain.handle('url-service:http-request', async (event, options: any) => {
    try {
 ...
 * 清理 IPC Handlers
 */
export function cleanupPhotoProxyIPC(): void {
  if (photosService) {
