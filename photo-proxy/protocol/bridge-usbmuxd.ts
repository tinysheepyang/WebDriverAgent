...
export class BridgeServiceUsbmuxd {
   ...
   * 处理消息
   */
  private processMessages(buffer: Buffer): Buffer {
    // 调试：打印收到的原始数据
    if (buffer.length > 0) {
    ...
    while (buffer.length >= 8) {
      ...
      let binaryData: Buffer | undefined = undefined
  ...
  private async sendRequest(type: MessageType, payload: any, expectStream: boolean = false): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.serviceSocket!.write(buffer, (error: Error | undefined) => {
        if (error) {
          clearTimeout(timeout)
          this.pendingRequests.delete(messageId)
