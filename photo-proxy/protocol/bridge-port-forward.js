...
import { FrameSerializer, FrameType } from './frame.js';
...
export class BridgeServicePortForward {
    constructor() {
        ...
        this.forwardPort = 12345;
        this.receiveBuffer = Buffer.alloc(0);
    }
    /**
     * 清理可能存在的旧端口转发进程
     */
    async cleanupExistingPortForward() {
        try {
            // 检查端口是否被占用
     ...
     * 建立端口转发
     */
    async setupPortForward(udid) {
        // 先检查并清理可能存在的旧进程
        await this.cleanupExistingPortForward();
        return new Promise((resolve, reject) => {
            const iosPath = getIOSCommandPath();
            const cwd = process.env.IOS_COMMAND_PATH || (typeof global !== 'undefined' && global.electronApp && global.electronApp.isPackaged)
    ...
    async sendRequest(type, payload, expectStream = false) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.serviceSocket.write(frame, (error) => {
                if (error) {
                    clearTimeout(timeout);
                    this.pendingRequests.delete(requestId);
