...
import { LockdowndClient, type LockdowndService } from '../usb/lockdownd.js'
import { MessageSerializer, MessageType, type Message, type ListAssetsRequest, type GetThumbnailRequest, type GetImageRequest, type GetVideoStreamRequest, type OpenURLRequest } from './message.js'
import type { IOSDevice } from '../usb/device.js'
import { Readable } from 'stream'

export class BridgeService {
  private usbmuxd: UsbmuxdClient | null = null
  private lockdownd: LockdowndClient | null = null
  ...
  async connect(device: IOSDevice): Promise<void> {
    ...
    let service: LockdowndService | null = null
   ...
   * 发送消息并等待响应
   */
  private async sendRequest(type: MessageType, payload: any, expectStream: boolean = false): Promise<any> {
  ...
  async listAssets(limit: number, offset: number): Promise<{ total: number; items: any[] }> {
    const request: ListAssetsRequest = { limit, offset }
  ...
  async getThumbnail(assetId: string, size: number): Promise<Buffer> {
    const request: GetThumbnailRequest = { assetId, size }
   ...
   * 获取原图（流式）
   */
  async getImage(assetId: string, quality: 'full' | 'screen', format: 'jpeg' | 'heic'): Promise<Readable> {
    const request: GetImageRequest = { assetId, quality, format }
    return this.sendRequest(MessageType.GET_IMAGE, request, true) as Promise<Readable>
  }
  ...
  async getVideoStream(assetId: string): Promise<Readable> {
    const request: GetVideoStreamRequest = { assetId }
    return this.sendRequest(MessageType.GET_VIDEO_STREAM, request, true) as Promise<Readable>
  }
  ...
  async openURL(url: string): Promise<void> {
    const request: OpenURLRequest = { url }
